...
# 3) AutoMathAccessibilityService.kt (overwrite with template support)
...
if "Save digit template" not in s:
    ...
    if idx != -1:
        ...
        while i < len(s) and depth > 0:
            ...
            i += 1
        body = s[br+1:i-1]
        if "R.id.btnGrant).setOnLongClickListener" not in body:
            body = body + "\n" + inject
            s = s[:br+1] + body + s[i-1:]
            p.write_text(s, encoding="utf-8")
            print("[+] Injected long-press dialog on Grant button.")
        else:
            print("[=] Long-press dialog already present.")
    else:
        print("[!] Could not find onCreate() in MainActivity.kt; no injection.")
else:
    print("[=] Save Template dialog already present.")
PY

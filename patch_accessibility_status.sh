...
companion object {  
    ...
    /** فحص قوي: يستخدم قائمة الخدمات المفعّلة من AccessibilityManager */  
    fun isEnabled(ctx: Context): Boolean {  
        ...
        for (info in list) {  
            ...
            val compOk = (si.packageName == myPkg && si.name == myCls)  
            if (compOk) return true  
        }  
        // احتياطياً: جرّب قراءة Settings.Secure في حالة الأنظمة القياسية  
